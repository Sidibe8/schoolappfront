...
import { addClasse, deleteClasse, updateClasse } from '../../actions/classeActions';
...
function ClassePage() {
  ...
  const [editedClasseId, setEditedClasseId] = useState(null);
  ...
  const handleEdit = (className) => {
    setEditedClassName(className.nom);
    setEditedClasseId(className._id);
    setIsEditing(true);
  };
  ...
  const handleUpdate = () => {
    dispatch(updateClasse(editedClasseId, { nom: editedClassName }));
    setEditedClassName('');
    setIsEditing(false);
  };
  ...
  const handleAdd = () => {
    if (editedClassName.trim() !== '') {
      dispatch(addClasse({ nom: editedClassName }));
      setEditedClassName('');
    }
  };
                ...
                <div className="row align-items-center">
                  ...
                  <div className="col-auto">
                    ...
                    <Link to="#" className="text-decoration-none" onClick={() => handleEdit(item)}>
                      <i className="fas fa-pencil-alt text-secondary" style={{ fontSize: 20 }}></i>
                    </Link>
