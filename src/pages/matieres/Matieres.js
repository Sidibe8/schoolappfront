...
import { addMatiere, deleteMatiere, deleteNotesByMatiere, updateMatiere } from '../../actions/matiereActions';
...
function Matieres() {
  ...
  const [editedMatiereId, setEditedMatiereId] = useState(null); // State pour stocker l'ID de la matière à éditer
  ...
  if (!Array.isArray(matiere)) {
    return (
      <div className='container my-4 d-flex justify-content-center align-items-center flex-column' style={{height: "70vh"}}>
        ...
        <br />
        <small>No matiere to show</small>
      </div>
    );
  }

  const filteredData = matiere.filter((item) =>
  ...
  const handleEdit = (matiere) => {
    setEditedClassName(matiere.nom);
    setEditedMatiereId(matiere._id); 
    setIsEditing(true);
  };

  const handleCancel = () => {
    setEditedClassName('');
    setEditedMatiereId(null); // Réinitialiser l'ID de la matière à éditer
    setIsEditing(false);
  };

  const handleUpdate = () => {
    dispatch(updateMatiere(editedMatiereId, { nom: editedClassName })); // Passer l'ID et le nom mis à jour
    setEditedClassName('');
    setEditedMatiereId(null); // Réinitialiser l'ID de la matière à éditer
    setIsEditing(false);
  };
  ...
  const handleAdd = () => {
    if (editedClassName.trim() !== '') {
      dispatch(addMatiere({ nom: editedClassName }));
      setEditedClassName('');
    }
  };
  ...
  const handleDelete = (id) => {
    ...
    if (confirmDelete) {
      dispatch(deleteNotesByMatiere(id));
              ...
              <div className="card-body">
                <div className="row align-items-center">
                  <div className="col mr-2">
                    {/* <div className="text-xs font-weight-bold text-uppercase mb-1">Matiere</div> */}
                    <div className="h5 mb-0 font-weight-bold text-gray-800" style={{ fontSize: '18px' }}>
                      {item.nom}
                    </div>
                  </div>
                  <div className="col-auto">
                    {/* <Link to={`/classe/${item._id}`} className="text-decoration-none me-2">
